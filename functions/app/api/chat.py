...
async def handle_chat(request: ChatRequest = Body(...), _: None = Depends(rate_limit)):
    """Handles incoming chat messages and returns the bot's RAG-based reply."""
    print("⏱ Time since process start:",
      round(time.time() - START_TIME, 2), "sec")

    # new code
    try:
        response = await chat_manager.process_chat(request)
        return response
